...
| `34-diving-emergencies` | צלילה, דקומפרסיה, ברוטראומה | `scene_assessment`, `diving_emergency_overview`, `diving_history`, `diving_barotrauma_lung`, `diving_decompression`, `diving_bls_management` | מכוסה | נוספה כעת גם כניסת זירה מוקדמת לתאונת צלילה, ולא רק דרך `secondary_survey`. |
