...
| UIX-405 | done | P2 | תבנית "אזור מיידי + אקורדיונים" עדיין ארוכה מדי מעל ה-fold | גם אחרי redesign phases, במסכים ראשונים עדיין נדרשת גלילה מוקדמת מדי כדי לראות את התוכן שמוגדר כ-"Immediate". זה מצביע על vertical rhythm שעדיין לא אופטימלי. | spec קובע `Immediate content first`; בבדיקות מסך האחרונות כמות chrome לפני התוכן עדיין גבוהה. | לבצע audit ממוקד ל-vertical rhythm: גובה hero, ריווח בין sections, גובה כותרות, ותמצות בלוקים קבועים במובייל. | Medium | 1-2 ימים | במסך ראשון התוכן המיידי מגיע מהר יותר לעין, בלי לפגוע בהבנה ובאמון. | השוואת before/after על 2-3 צמתים מייצגים במובייל, יחד עם `lint` + `build` + `test`, ואימות ב-`393x852` לוקאלית ובפרודקשן. |
...
- `2026-07-06` - `UIX-405` נסגר: אזור ה-hero במובייל כווץ, כרטיסי הסיכום עברו לתצורת compact מתחת ל-"מה עושים עכשיו", ו-padding של האקורדיונים הוקטן כדי להביא את התוכן המיידי גבוה יותר מעל ה-fold.
- `2026-07-06` - `UIX-405` אומת לוקאלית ב-`127.0.0.1:4175` עם viewport של `393x852`, לאחר `lint` + `build` + `test`.
- `2026-07-06` - `UIX-405` אומת גם בפרודקשן החי על `https://bls-protocol.evyatarhazan.com/` לאחר ש-`Validate` לריצה `28779465060` הסתיים בהצלחה, וה-snapshot במובייל הראה ש-"מה עושים עכשיו" מופיע לפני כרטיסי summary הארוכים.
